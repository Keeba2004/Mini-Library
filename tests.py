...
def test_borrow_when_no_copies_left():
    ...
    # Add a member
    add_member("A001", "Jane Conteh", "jane@example.com")
    
    # Borrow the only copy
    borrow_book("A001", "978-1234567890")
    
    # Try to borrow again (should fail)
    result = borrow_book("A001", "978-1234567890")
...
def test_member_borrow_limit():
    ...
    assert borrow_book("A001", "978-1") == True, "First borrow should succeed"
    assert borrow_book("A001", "978-2") == True, "Second borrow should succeed"
    assert borrow_book("A001", "978-3") == True, "Third borrow should succeed"
    
    # Try to borrow 4th book (should fail)
    result = borrow_book("A001", "978-4")
    assert result == False, "Should not be able to borrow more than 3 books"
    
    # Check member has exactly 3 books
    member = next(m for m in members if m["member_id"] == "A001")
...
def test_search_books():
    ...
    add_book("978-3", "Wuthering Heights", "Emily Bronte", "Fiction", 1)
    ...
    assert len(results) == 1, "Should find one book by Bronte"
...
def test_return_book():
    ...
    # Add book and member
    add_book("978-1234567890", "The Great Gatsby", "F. Scott Fitzgerald", "Fiction", 1)
    add_member("D001", "Jane Conteh", "Jane@example.com")
    ...
    # Return the book
    result = return_book("D001", "978-1234567890")
    ...
    member = next(m for m in members if m["member_id"] == "D001")
